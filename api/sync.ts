...
// @ts-expect-error -- Vercel bundles TypeScript function imports before Node execution.
import { applyCors, requireMethod } from "./_lib/http.ts";
// @ts-expect-error -- Vercel bundles TypeScript function imports before Node execution.
import { hasValidSession } from "./_lib/session.ts";

type SyncAction = "save" | "read";

const DEFAULT_SYNC_SUPABASE_URL = "https://ecybgsrxjnmifahtbazk.supabase.co";
const DEFAULT_SYNC_SUPABASE_PUBLISHABLE_KEY = "sb_publishable_Gp3VCnJmsPi0J5NwGdxRgw_iKNFPY11";
...
export default async function handler(
  ...
  if (
    ...
    response.status(400).json({ error: "The encrypted sync backup is invalid." });
    return;
  }
  ...
  try {
    ...
    response.status(502).json({ error: "Secure sync is temporarily unavailable." });
  }
}
